    ...
    if content_area:
        for img in content_area.find_all("img"):
            src = (img.get("src") or img.get("data-src") or
                   img.get("data-lazy-src") or img.get("data-orig-file") or "")
            if not src: continue
            s = src.lower()
            if any(b in s for b in ["logo","icon","avatar","gravatar","banner","author","social","spinner"]): continue
            try:
                if int(img.get("width","999")) < 150: continue
            except: pass
