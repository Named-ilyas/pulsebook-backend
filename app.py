...
Added: clinical field, edit endpoint, base64 image support
"""
...
def init_db():
    ...
    """)
    # Add clinical column if it doesn't exist (migration for existing DBs)
    try:
        conn.execute("ALTER TABLE ecgs ADD COLUMN clinical TEXT DEFAULT ''")
        conn.commit()
    except:
...
@app.route("/api/ecgs/<ecg_id>", methods=["PUT"])
def update_ecg(ecg_id):
    data = request.json or {}
    conn = get_db()
    row  = conn.execute("SELECT id FROM ecgs WHERE id=?", (ecg_id,)).fetchone()
    if not row:
        conn.close()
        return jsonify({"error": "Not found"}), 404

    allowed = ["image_url", "summary", "clinical", "rate", "rhythm",
               "axis", "intervals", "features", "pearls"]
    ...
    values     = list(updates.values()) + [ecg_id]
    conn.execute(f"UPDATE ecgs SET {set_clause} WHERE id=?", values)
    conn.commit()
...
def receive_ecg():
    ...
    conn.execute("""
        ...
        (id,category,subcategory,difficulty,site,source_url,image_url,
         rate,rhythm,axis,intervals,summary,features,pearls,clinical,scraped_at)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
    """, (
        ...
        ecg.get("clinical",""),  time.strftime("%Y-%m-%dT%H:%M:%S")
    ))
    conn.commit()
    conn.close()
    return jsonify({"ok": True})
