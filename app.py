    ...
    try:
        ...
        try:
            if request.method == "POST":
                ...
                conn.execute("INSERT INTO lessons (id, category_id, title, content_html, pdf_url, created_at) VALUES (?, ?, ?, ?, ?, ?)",
                            (lid, data.get("category_id"), data.get("title"), data.get("content_html"), data.get("pdf_url"), time.strftime("%Y-%m-%d %H:%M:%S")))
                conn.commit()
...
@app.route("/api/lessons/<lid>", methods=["DELETE", "PUT"])
def sub_manage_lesson(lid):
    try:
        ...
        try:
            ...
            if request.method == "PUT":
                data = request.json
                conn.execute("UPDATE lessons SET title=?, content_html=?, pdf_url=? WHERE id=?",
                            (data.get("title"), data.get("content_html"), data.get("pdf_url"), lid))
                conn.commit()
                return jsonify({"ok": True})
        finally:
            conn.close()
