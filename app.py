...
def get_categories():
    conn = get_db()
    rows = conn.execute("SELECT category, COUNT(*) as n FROM ecgs GROUP BY category ORDER BY n DESC").fetchall()
    conn.close()
    return jsonify([dict(r) for r in rows])
...
def get_stats():
    ...
    conn.close()
    return jsonify({"total": total, "version": "1.0.0"})
...
def trigger_scrape():
    """Called by the scraper script to upload a new ECG"""
    ecg = request.json
    ...
    conn.execute("""
        ...
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
    """, (
        ecg.get("id"), ecg.get("category"), ecg.get("subcategory"),
        ecg.get("difficulty","intermediate"), ecg.get("site",""),
        ecg.get("source_url",""), ecg.get("image_url",""),
        ecg.get("rate",""), ecg.get("rhythm",""), ecg.get("axis",""),
